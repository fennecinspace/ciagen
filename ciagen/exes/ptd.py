...
class PTD:
    ...
    def __call__(self, paths: Dict[str, str | Path]) -> None:
        ...
        # Loading real images
        real_images, real_image_names = loading_images(real_path_images)
        ...
        logger.info(f"Using {len(synthetic_images)} Synthetic images from: {generated_path}")
        ...
        for metric in self.cfg['metrics']['ptd']:
            ...
            for image_iter in range(len(synthetic_images)):
              result[synthetic_image_names[image_iter]] = scores[image_iter]
        print(result)
