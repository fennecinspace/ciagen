...
def create_transform_dict(cfg: DictConfig) -> Dict[str, Callable[[Any], torch.Tensor]]:
    return {
        fe: instance_transform(fe) for fe in cfg["metrics"]["fe"]
    }


def call_dataloader2(paths: Dict[str, str | Path], cfg: DictConfig, feature_extractor_name: str, transform_dict: Dict[str, Callable[[Any], torch.Tensor]], is_real: bool) -> None:
