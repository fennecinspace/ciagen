    ...
    def __call__(
        ...
        transform = transforms.Compose(
            [
                ...
                ),  # Normalization for InceptionV3
            ]
        )

        # Create Data Loaders
        self.train_dataset = CSVDataframeDataset(
            train_df, transform=transform, labels_column_title=labels_column_title
        )
        self.val_dataset = CSVDataframeDataset(
            val_df, transform=transform, labels_column_title=labels_column_title
        )
        self.test_dataset = CSVDataframeDataset(
        ...
        filtering_strategy = self.cfg["filtering"]["type"]
    ...
    def train_model(self, epochs: int = 10):
        ...
        for epoch in range(epochs):
            ...
            train_loader_tqdm = tqdm(
                self.train_loader, desc=f"Epoch {epoch + 1}/{epochs} [Training]"
            )
