...
from common import ciagen_logger, create_yaml_file
...
def main(cfg: DictConfig) -> None:
    ...
    for i, run in enumerate(runs_info):
        ...
        if run["map"] != 0:
            print(
                f'[{i}/{len(runs_info)}]: Skipping {run["run_name"]}, already tested with an map of {run["map"]}'
            )
            continue

        print(
            f'[{i}/{len(runs_info)}]: Testing {run["run_name"]}, already tested with an map of {run["map"]}'
        )
