...
class MLD(QualityMetric):
    """Mahalanobis distance for each generated image against the real distribution."""
    ...
    def score(
        ...
        real_mean = self._mean_calculator.state().to(self.device)
        real_cov = self._cov_calculator.state().to(self.device)
