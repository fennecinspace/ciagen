...
from common import ciagen_logger
...
class Downloader:
    ...
    def get_runs(self, query_filter=None):
        ...
        ciagen_logger.info("Loading Runs DB...")
        if query_filter:
            finished_query = self.api.runs(
        ...
        ciagen_logger.info("Loading Runs to memory...")
    ...
    def check_runs(self, runs, query_filter):
        ciagen_logger.info("Checking list of finished runs...")
        ...
        for run in runs:
            if type(run).__name__ == "str":
                ciagen_logger.info(f"Could not find {run}")
            else:
                runs_found += [run]
        return runs_found
    ...
    def download_model(self, run, download_dir=DEFAULT_DOWNLOAD_DIR):
        ...
        if os.path.exists(download_location):
            ciagen_logger.info(f"Skipping : '{run.id}.{run.name}.pt' already exists")
        else:
            try:
                model_artifact = self.api.artifact(
                    f"{self.entity}/{self.project}/run_{run.id}_model:best"
                )
                ...
                if os.path.exists(os.path.join(download_dir, "best.pt")):
                    ...
                    ciagen_logger.info("Skipping : Could not download.")
            except Exception as e:
                ciagen_logger.info(f"Could not download {run}. E:{e}")
    ...
    def save_summary(self, runs, download_dir=DEFAULT_DOWNLOAD_DIR, project="results"):
        ...
        ciagen_logger.info(f"Saving WANDB summary to: {save_path}")
...
def main(cfg: DictConfig) -> None:
    ...
    query_filter = (
        None if not download_params["query_filter"] else download_params["query_filter"]
    )
    ...
    for run in runs_to_process:
        ...
        runs_to_print += [
            [run.id, run.name, run.state, downloaded, downloader.runs_url + run.id]
        ]

    ciagen_logger.info(
        tabulate(runs_to_print, headers=["ID", "NAME", "STATUS", "DOWNLOADED", "URL"])
    )

    if download_params["list_all"]:
        ciagen_logger.info(
            f"Models: {len(finished + running + other)}, Downloaded: {downloaded_nb} (In : {folder}). Can not download {len(running)}, still running."
        )
    elif download_params["list_finished"] or "runs" in download_params:
        ciagen_logger.info(
            f"Models: {len(finished)}, Downloaded: {downloaded_nb} (In : {folder})"
        )
    elif download_params["list_running"]:
        ciagen_logger.info(
            f"Models: {len(running)}. Still running. Not able to download weights yet."
        )

    if download_params["download"] and runs_to_download:
        for run in runs_to_download:
            ciagen_logger.info(f"Downloading : {run.id}.{run.name}.pt")
            downloader.download_model(run, folder)

    try:
        ciagen_logger.info("Saving Runs summary !")
        downloader.save_summary(runs_to_process, folder, project)
    except Exception as e:
        ciagen_logger.info("Could not save summary.")
        ciagen_logger.info(e)
